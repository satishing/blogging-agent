...
| `tasks/editing_task.py`          | Editorial pass; serializes via `output_pydantic=BlogDraft`                                                | demo Step 05               |
| `tasks/publishing_task.py`       | Publish-to-Dev.to task                                                                                     | demo Step 06               |
...
2. **`agents/`** — planner, writer, editor, publisher. Each file has a docstring linking to its demo equivalent.
3. **`tasks/`** — `outline_task` plans, `writing_task` carries the readability spec + a deterministic guardrail (length band, required sections, inline citations) with bounded retries, `editing_task` does a real edit pass and serializes through `output_pydantic`.
4. **`tools/`** — note the `Client` split: HTTP transport only. `SerperSearchClient` parses ISO/relative/year dates and keeps undated results (with `published_date=None`). `DevToPublisherTool` takes a `PublishingService` so idempotency holds even when an agent publishes.
5. **`services/crew_service.py`** — the heart of the file:
   - `_run_content_pipeline` — gather sources, run the outline→write→edit crew once, override `blog.sources` with the validated set, then finalize references and read-time.
   - `_run_publish_pipeline` — the demo Step 06 multi-agent publish flow, plus an idempotency-cache fallback for when the agent's text output isn't valid JSON.
...
| Structured output | `tasks/editing_task.py` + `models/blog.py` | `output_pydantic=BlogDraft` validates the schema and retries on malformed output; the model enforces a minimum word count. |
...
46 tests covering:
- Agent / task factory contracts and the writing guardrail logic
...
- Idempotency (`PublishingService`) and the multi-agent publish flow with junk-output recovery
- Pipeline orchestration: gather → run crew once → override sources
